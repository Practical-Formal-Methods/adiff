...
instance Persistent VerifierRun where
  persist conn run = do
    let row = [toField (runVerifierName run), toField (verifierResult run), toField (verifierCode run)]
    SQL.execute conn "INSERT INTO runs(verifier_name,result,code_hash) VALUES (?,?,?)" row
