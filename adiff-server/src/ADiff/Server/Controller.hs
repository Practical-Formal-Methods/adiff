...
module ADiff.Server.Controller where

import           ADiff.Server.Prelude
import           ADiff.Server.Widgets
...
getIndex = do
  ...
  defaultLayout "ADiff " $(shamletFile "templates/index.hamlet")
  where
    ...
    sampleLinkDeltaUnsoundness = "/findings?q=" <> JSON.encodeToLazyText (Q2.Delta (Just "smack") Q2.SuspicionUnsound defaultWeights 2)
