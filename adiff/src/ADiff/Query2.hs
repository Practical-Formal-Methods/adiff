...
import           ADiff.Prelude                            hiding (Disagreement)
  ...
  where
    weightExists w = runBeam $ do
      (Just (exists :: Bool)) <- runSelectReturningOne $ select $ return $ exists_ $
        filter_ (\c -> (c ^. consensusWeights) ==. val_ w) $ all_ (adiffDb ^. tmpConsensus)
      return exists
...
allPrograms = runBeam $ runSelectReturningList $ select $ all_ (adiffDb ^. programs)

getProgramByHash :: (HasDatabase env, HasLogFunc env) => Text -> RIO env (Maybe Program)
getProgramByHash hsh = runBeam $ runSelectReturningOne $ lookup_ (adiffDb ^. programs) (toProgramId hsh)
...
getRunById i = runBeam $ runSelectReturningOne $ lookup_ (adiffDb ^. runs) i
...
storeProgram p = do
  ...
  unless exists $ runBeam $ runInsert $ insert (adiffDb ^. programs) $ insertValues [p]

storeRun :: (HasDatabase env, HasLogFunc env) => VerifierRun -> RIO env ()
storeRun r = runBeam $ runInsert $ insert (adiffDb ^. runs) $ insertValues [r]

storeRunFreshId :: (HasDatabase env, HasLogFunc env) => VerifierRun -> RIO env VerifierRun
storeRunFreshId r = do
  [run] <- runBeam $ runInsertReturningList (adiffDb ^. runs) $ insertExpressions
...
tagRun rid pairs = runBeam $ runInsert $ insert (adiffDb ^. tags) $ insertExpressions $
...
tagProgram hsh pairs = runBeam $ runInsert $ insert (adiffDb ^. tags) $ insertExpressions $
...
lookupRun vn hs = runBeam $ runSelectReturningOne $ select s
  where
    s = filter_ (\r -> (r ^. program) ==. val_ hs &&. r ^. verifierName ==. val_ vn) $ all_ (adiffDb ^. runs)


updateRun :: (HasDatabase env, HasLogFunc env) => VerifierRun  ->  RIO env ()
updateRun r = runBeam $ runUpdate $ save (adiffDb ^. runs) r

-- | morally produces [Finding]
findingsByVerdicts :: [(Relatee, [Verdict])] -> Q _ ADiffDb ctx (PrimaryKey ProgramT _,_,_,_,_,_)
findingsByVerdicts rvs = agg $ do
  -- the program
  p <- programByVerdicts rvs
  -- join with all runs that say "Sat"
  sats <- leftJoin_ (filter_ (\r -> (r ^. resultVerdict) ==. val_ Sat) $ all_ $ adiffDb ^. runs)
            (\r -> (r ^. program) ==. (p ^. hash) )

  -- join with all runs that say "Unsat"
  unsats <- leftJoin_ (filter_ (\r -> (r ^. resultVerdict) ==. val_ Unsat) $ all_ $ adiffDb ^. runs)
            (\r -> (r ^. program) ==. (p ^. hash) )

  return (pk p, p ^. origin, sats ^. verifierName, unsats ^. verifierName)
  where
    consensusByWeight w = filter_ (\c -> (c ^. consensusWeights) ==. val_ w) (all_ $ adiffDb ^. tmpConsensus)
...
findingsByDelta :: Maybe VerifierName -> Suspicion -> Weights -> Int -> Q _ ADiffDb ctx _
findingsByDelta mvn s weights delta = agg $ do
  -- the program
  p <- all_ (adiffDb ^. programs)
  ...
  consensus <- filter_ (\c -> (c ^. consensusWeights) ==. val_ weights ) $ all_ (adiffDb ^. tmpConsensus)
...
calculateConsensus w@(Weights consensusAlgorithm weights) = do
  -- runBeam $ runDelete $ delete (adiffDb ^. tmpConsensus) (const $ val_ True)
  let bs = 100 :: Int
  logInfo $ "updating consensus table (using batch size " <> display bs <> ")"

  (Just numPrograms ) <- runBeam $ runSelectReturningOne $ select $ aggregate_ (const countAll_) $ all_ (adiffDb ^. programs)
  ...
  forM_ [0.. numPrograms `div` bs + 1] $ \i -> do
    ...
    runBeam $ runInsert $ insert (adiffDb ^. tmpConsensus) $ insertFrom $ do
      p <- limit_ (fromIntegral bs) $ offset_ (fromIntegral $ i * bs) $ all_ (adiffDb ^. programs)
...
removeSubsumedUnknowns = do
  ...
  -- runBeam $ runDelete $ delete (adiffDb ^. runs) $ \r -> exists_ $ do
  --   t <- all_ (adiffDb ^. runs)
...
tagsForProgram hsh = runBeam $ runSelectReturningList $ select $ project <$> relevantTags
  where
    ...
    relevantTags = filter_ (\t -> (t ^. taggedProgramId) ==. val_ (Just hsh)) $ all_ (adiffDb ^. tags)

--------------------------------------------------------------------------------
-- query fragments

allRuns_ :: Q _ _ _ (VerifierRunT (QExpr _ _))
allRuns_      = all_ (adiffDb ^. runs)

allPrograms_ :: Q _ _ _ (ProgramT (QExpr _ _))
allPrograms_  = all_ (adiffDb ^. programs)
...
programByVerdicts specs = nub_ $ do
  ...
  forM_ specs $ \(relatee, vrds) ->
    case relatee of
      ...
      (ConsensusBy w) -> do
        c <- filter_ (\c -> (c ^. consensusWeights) ==. val_ w ) $ all_ (adiffDb ^. tmpConsensus)
